...
const compareFilesData = (object1, object2) => {
    ...
    for (const key of allKeys) {
        ...
        if (keys1.includes(key) && keys2.includes(key)) {
            value1 === value2 ? resultArray.push({symbol: ' ', key: key, value: value1, file: 1}) : resultArray.push({symbol: '-', key: key, value: value1, file: 1}, {symbol: '+', key: key, value: value2, file: 2});

        }else if (keys1.includes(key)) {
            resultArray.push({symbol: '-', key: key, value: value1, file: 1});

        } else {
            resultArray.push({symbol: '+', key: key, value: value2, file: 2});
        }
    };
